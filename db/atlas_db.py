...
# ── Strategy Lifecycle (promotion stage) ────────────────────────────────────
# Tracks (strategy, universe) tuples through RESEARCH → PAPER → LIVE → RETIRED.
# Separate from monitor/lifecycle.py which tracks operational health of LIVE
# strategies (RAMP_UP / ACTIVE / WATCH / PROBATION / SUSPENDED).

import logging as _lifecycle_logging
_lifecycle_log = _lifecycle_logging.getLogger(__name__)
...
def get_lifecycle_state(strategy: str, universe: str) -> Optional[str]:
    ...
    try:
        ...
        _lifecycle_log.warning("get_lifecycle_state(%s, %s) failed: %s", strategy, universe, exc)
        return None
...
def set_lifecycle_state(
    ...
    Atomically:
      1. Reads current state + paper dates from strategy_lifecycle.
      2. Upserts strategy_lifecycle row with new state, prev_state, timestamps.
      3. Inserts a strategy_lifecycle_history row.

    paper_start_date is set (if not already set) when transitioning TO PAPER.
    paper_end_date is set when transitioning FROM PAPER to any other state.

    Raises ValueError if new_state is not a valid lifecycle state.
    """
...
def list_lifecycle_states(state: Optional[str] = None) -> List[Dict]:
    ...
    Returns a list of dicts with keys:
        strategy, universe, state, entered_state_at, prev_state,
        transition_reason, paper_start_date, paper_end_date, auto_promotion_id, notes
    """
    try:
        with get_db() as db:
