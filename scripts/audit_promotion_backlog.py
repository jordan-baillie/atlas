...
def _get_research_best_sharpe(strategy: str, universe: str, conn: sqlite3.Connection) -> Optional[float]:
    """Return current research_best sharpe for (strategy, universe), or None."""
    row = conn.execute(
        "SELECT sharpe FROM research_best WHERE strategy=? AND universe=?",
        (strategy, universe),
    ).fetchone()
...
def main() -> None:
    ...
    for row in rows:
        ...
        current_best_str = f"{current_best:.4f}" if current_best is not None else "N/A"
