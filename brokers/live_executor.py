...
from brokers.preflight import (
    PreflightError,
    preflight_check_config,
    preflight_check_order,
    is_already_protected as _is_already_protected,  # back-compat alias for 3 test files
    protective_ledger_enabled as _protective_ledger_enabled,  # back-compat alias
)
...
# Preflight guards — moved to brokers/preflight.py (decomposition PR1.2)
...
class LiveExecutor:
    ...
    # (methods moved to brokers/execution_analytics.py — decomposition PR1.3)

    def get_fee_analysis(self, days: int = 90) -> dict:
        """Analyse actual fees vs assumed fees in config."""
        from brokers.execution_analytics import get_fee_analysis
        return get_fee_analysis(self._broker, days)

    def get_slippage_analysis(self, days: int = 90) -> dict:
        """Analyse actual slippage vs assumed slippage in config."""
        from brokers.execution_analytics import get_slippage_analysis
        return get_slippage_analysis(self._broker, days)

    def get_execution_history(self, days: int = 30) -> dict:
        """Full execution history with fees, slippage, and P&L per trade."""
        from brokers.execution_analytics import get_execution_history
        return get_execution_history(self._broker, days)
