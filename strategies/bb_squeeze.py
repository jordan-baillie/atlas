...
from indicators import calc_atr, calc_volume_ratio
    ...
    def generate_signals(
        ...
        for ticker, df in data.items():
            ...
            try:
                ...
                if not (squeeze_on.iloc[prev] and not squeeze_on.iloc[curr]):
                    continue

                # Momentum check: linear regression slope must be positive
                mom_slice = close.iloc[curr - self.momentum_period + 1: curr + 1].values
                ...
                # BB expansion bonus
                bb_width_curr = float((bb_upper.iloc[curr] - bb_lower.iloc[curr]) / bb_ma.iloc[curr]) if float(bb_ma.iloc[curr]) > 0 else 0
                bb_width_prev = float((bb_upper.iloc[prev] - bb_lower.iloc[prev]) / bb_ma.iloc[prev]) if float(bb_ma.iloc[prev]) > 0 else 0
                ...
                rationale = (
                    ...
                    f"slope={slope:.4f}, vol_ratio={vol_ratio.iloc[curr]:.2f}"
                )
    ...
    def check_exits(
        ...
        for ticker, pos, df in self._iter_my_positions(data, existing_positions):
            ...
            try:
                ...
                # 3. Signal exit: momentum slope turns negative
                if exit_reason is None:
                    mom_slice = close.iloc[-self.momentum_period:].values
                    if len(mom_slice) >= self.momentum_period:
                        slope = np.polyfit(range(len(mom_slice)), mom_slice, 1)[0]
                        if slope < 0:
