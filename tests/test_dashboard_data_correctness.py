...
def test_equity_curve_uses_alpaca_portfolio_history(tmp_path, monkeypatch):
    ...
    # Must have at least 5 entries (the 5 from Alpaca + possibly today appended)
    assert len(ph) >= 5, f"Expected ≥5 rows, got {len(ph)}"
    ...
    assert ph[0]["date"] == fake_history[0]["date"]
    assert ph[0]["equity"] == fake_history[0]["equity"]
