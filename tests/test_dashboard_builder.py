...
def _patch_db(conn: sqlite3.Connection):
    """Patch db.atlas_db.get_db to yield the given in-memory connection."""
    ...
    with patch("db.atlas_db.get_db", _fake):
        yield
...
class TestFetchBrokerState:
    ...
    def test_returns_all_required_keys(self):
        from services.api.dashboard_builder import fetch_broker_state
...
class TestBuildAccountSection:
    ...
    def test_num_positions_from_positions_list(self, tmp_path):
        from services.api.dashboard_builder import build_account_section
...
class TestBuildPositionsSection:
    ...
    def test_pass1_trade_metadata_from_sqlite(self):
        from services.api.dashboard_builder import build_positions_section
    ...
    def test_pass2_alpaca_intraday_enrichment(self):
        ...
        conn = _make_minimal_db()
        with _patch_db(conn):
            with patch("brokers.alpaca.mapper.to_alpaca", return_value="AAPL"):
                result = build_positions_section(
...
class TestBuildOrdersSection:
    ...
    def test_flattens_raw_fields(self):
        from services.api.dashboard_builder import build_orders_section
...
class TestBuildEquityCurveSection:
    ...
    def test_raw_data_passthrough(self):
        from services.api.dashboard_builder import build_equity_curve_section

        raw = [
            {"date": "2026-04-01", "equity": 5000.0, "value": 5000.0, "day_pnl": 10.0},
            {"date": "2026-04-02", "equity": 5010.0, "value": 5010.0, "day_pnl": 10.0},
        ]
...
class TestBuildStrategyStats:
    ...
    def test_strategy_performance_from_closed_trades(self):
        from services.api.dashboard_builder import build_strategy_stats
...
class TestBuildPnlSummary:
    ...
    def test_uses_alpaca_intraday_when_available(self):
        from services.api.dashboard_builder import build_pnl_summary
    ...
    def test_falls_back_to_tiingo_when_no_intraday(self, tmp_path):
        ...
        with patch("services.api.dashboard_builder._PROJECT_ROOT", tmp_path):
            result = build_pnl_summary(positions, "sp500", [], {})
...
class TestBuildDashboardDataShape:
    ...
    def reset_cache(self):
        from services.api import dashboard as dash_mod
    ...
    def test_broker_failure_returns_graceful_result(self, tmp_path, monkeypatch):
        ...
        with (
            patch("brokers.registry.get_live_broker", side_effect=RuntimeError("no broker")),
            _patch_db(conn),
        ):
            result = dash_mod._build_dashboard_data()
