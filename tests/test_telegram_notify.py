"""Tests for the utils.telegram.notify() convenience wrapper.
...
class TestNotifyNoLevel:
    ...
    def test_returns_false_when_send_message_returns_false(self):
        ...
        notify = _import_notify()
        with patch("utils.telegram.send_message", return_value=False):
            result = notify("msg")
...
class TestNotifyExceptionHandling:
    def test_exception_in_send_message_returns_false(self):
        ...
        notify = _import_notify()
        with patch("utils.telegram.send_message", side_effect=RuntimeError("network error")):
            result = notify("msg")
        assert result is False
    ...
    def test_exception_does_not_propagate(self):
        ...
        notify = _import_notify()
        with patch("utils.telegram.send_message", side_effect=ValueError("bad creds")):
            try:
                result = notify("msg")
            except Exception:
    ...
    def test_exception_logged_as_warning(self, caplog):
        ...
        import logging
        notify = _import_notify()
        with patch("utils.telegram.send_message", side_effect=OSError("timeout")):
            with caplog.at_level(logging.WARNING, logger="utils.telegram"):
    ...
    def test_category_appears_in_warning_log(self, caplog):
        ...
        import logging
        notify = _import_notify()
        with patch("utils.telegram.send_message", side_effect=ConnectionError("down")):
            with caplog.at_level(logging.WARNING, logger="utils.telegram"):
