...
Sub-modules:
    cache           -- Standalone constants + ticker-list helpers
    normalization   -- Ticker normalisation + OHLCV cleaning + split adjustments
    downloaders     -- yfinance + Alpaca download paths + routing
    sqlite_writer   -- SQLite batch write + integrity check
    freshness       -- Data freshness checks
    macro           -- Macro data refresh

Design note on cache I/O placement:
    CACHE_DIR, _market_cache_dir, _cache_path, _cache_is_fresh, _load_cache,
    _save_cache, clear_cache, cache_stats are defined directly in this module
    (not in cache.py) so that tests patching data.ingest.CACHE_DIR and
    data.ingest._market_cache_dir continue to work unchanged.  When functions
    are defined here, their global lookups resolve through data.ingest.__dict__,
    which is exactly the namespace tests patch.
"""
...
# ---------------------------------------------------------------------------
# Re-exports from sub-modules (standalone constants + helpers)
# ---------------------------------------------------------------------------

from data.ingest.cache import (  # noqa: E402
    CACHE_MAX_AGE_HOURS,
    DEFAULT_MARKET,
    _YFINANCE_ONLY,
    _is_crypto_ticker,
...
from data.ingest.normalization import (  # noqa: E402
    _normalize_ticker,
    _clean_ohlcv,
    _clean_alpaca_bars,
...
from data.ingest.downloaders import (  # noqa: E402
    YF_AVAILABLE,
    _download_via_yfinance,
    _download_via_alpaca,
    _fetch_ohlcv,
)

from data.ingest.sqlite_writer import (  # noqa: E402
    _sqlite_batch_write,
    verify_sqlite_integrity,
)

# freshness: _last_trading_day, check_data_freshness, verify_ingest_freshness
# defined directly below (after cache I/O) so test patches on data.ingest.* work.

from data.ingest.macro import (  # noqa: E402
    refresh_macro_data,
)

# ---------------------------------------------------------------------------
# Cache I/O -- defined here (not re-exported from cache.py) so that tests
# patching data.ingest.CACHE_DIR / data.ingest._market_cache_dir work.
# All internal calls use names from this module's globals, not cache.py's.
# ---------------------------------------------------------------------------

# data/ingest/__init__.py -> data/ingest/ -> data/ -> project root
...
# ---------------------------------------------------------------------------
# Orchestrator: download_universe
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Orchestrator: ingest_all_etf_universes
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Freshness checks -- defined here (not re-exported from freshness.py) so that
# tests patching data.ingest._last_trading_day continue to work.
# check_data_freshness calls _last_trading_day() via __init__.py globals, so
# patch("data.ingest._last_trading_day", ...) is visible to check_data_freshness.
# ---------------------------------------------------------------------------
