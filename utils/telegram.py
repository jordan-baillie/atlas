...
def _build_dashboard_from_sqlite() -> Optional[dict]:
    ...
    Returns the same structure as _build_dashboard_from_eod_summaries()
    so downstream consumers don't need changes.
    """
    try:
    ...
    try:
        # Get today's equity curve entries for all markets
        with atlas_db.get_db() as db:
            rows = db.execute(
                "SELECT * FROM equity_curve WHERE date=?", (today,)
            ).fetchall()
            if not rows:
                # Fall back to most recent date with data
                rows = db.execute(
                    "SELECT * FROM equity_curve ORDER BY date DESC LIMIT 10"
                ).fetchall()
            if not rows:
                return None
            summaries = {dict(r)["market_id"]: dict(r) for r in rows}

            # Get position snapshots — today or most recent
            pos_rows = db.execute(
                "SELECT * FROM position_snapshots WHERE date=?", (today,)
            ).fetchall()
            if not pos_rows:
                # Fall back to most recent snapshot date
                latest_snap_date = db.execute(
                    "SELECT MAX(date) FROM position_snapshots"
                ).fetchone()
                if latest_snap_date and latest_snap_date[0]:
                    pos_rows = db.execute(
            ...
            for pr in pos_rows:
                ...
                positions_by_market.setdefault(mid, []).append(p)
    except Exception as e:
        logger.warning("Failed to read SQLite dashboard data: %s", e)
        return None

    if not summaries:
        return None

    # Use sp500 as primary if available
    primary_id = "sp500" if "sp500" in summaries else next(iter(summaries))
    primary = summaries[primary_id]

    total_equity = sum(s.get("equity", 0) for s in summaries.values())
    total_cash = sum(s.get("cash", 0) for s in summaries.values())
    total_pnl_day = sum(s.get("day_pnl", 0) or 0 for s in summaries.values())
    total_positions = sum(s.get("positions_count", 0) or 0 for s in summaries.values())
    total_pnl_all = sum(s.get("total_pnl", 0) or 0 for s in summaries.values())
    total_pnl_pct = sum(s.get("total_pnl_pct", 0) or 0 for s in summaries.values())
    total_realized = sum(s.get("realized_pnl", 0) or 0 for s in summaries.values())

    # Build position list
    all_positions = []
    for mid, positions in positions_by_market.items():
        for pos in positions:
            all_positions.append({
                ...
                "days_held": pos.get("holding_days", 0),
                "market": mid,
            })
    ...
    return {
        ...
        "portfolio": {
            ...
            "today_pnl_aud": total_pnl_day,
            "num_open": total_positions,
            "open_positions": all_positions,
            "realized_pnl": total_realized,
        },
        "closed_trades": [],  # Closed trades not stored in equity_curve; available from trades table
        "benchmark_return_pct": 0,
        "benchmark_ticker": "SPY",
        "risk": {
            "exposure_pct": round(100 * (total_equity - total_cash) / total_equity, 1) if total_equity > 0 else 0,
...
def _read_dashboard_data() -> Optional[dict]:
    """Read dashboard data from available sources.

    Priority:
    1. SQLite equity_curve + position_snapshots (primary)
    2. EOD summary JSON files (fallback)
    3. dashboard-data.json (legacy)
    """
    # Primary: SQLite
    result = _build_dashboard_from_sqlite()
    if result:
        return result

    # Fallback: EOD summary JSON files
    result = _build_dashboard_from_eod_summaries()
    if result:
        return result
    ...
    if dash_path.exists():
        try:
            ...
            logger.warning("Failed to read dashboard-data.json: %s", e)

    return None


def _build_dashboard_from_eod_summaries() -> Optional[dict]:
    """Build dashboard-compatible data from EOD summary JSON files.

    Reads eod_summary_{date}.json for all markets and constructs
    the data structure expected by _build_combined_snapshot().
    """
    from datetime import datetime
    today = datetime.now().strftime("%Y-%m-%d")

    # Collect all EOD summaries for today
    summaries = {}
    for summary_file in (PROJECT_ROOT / "logs").glob(f"eod_summary_{today}.json"):
        try:
    ...
    for market in ("sp500", "commodity_etfs", "sector_etfs", "asx"):
        path = PROJECT_ROOT / "logs" / f"eod_summary_{today}_{market}.json"
        if path.exists() and market not in summaries:
            try:
                with open(path) as f:
    ...
    if not summaries:
        logger.warning("No EOD summaries found for %s", today)
        return None

    # Build combined data from summaries
    # Use sp500 as primary if available, else first market
    primary = summaries.get("sp500") or next(iter(summaries.values()))

    total_equity = sum(s.get("equity", 0) for s in summaries.values())
    total_cash = sum(s.get("cash", 0) for s in summaries.values())
    total_pnl = sum(s.get("daily_pnl", 0) for s in summaries.values())
    total_positions = sum(s.get("positions", 0) for s in summaries.values())

    # Build position list from all markets
    all_positions = []
    all_closed = []
    for mid, s in summaries.items():
        for pos in s.get("position_details", []):
            pos["market"] = mid
            all_positions.append({
                "ticker": pos.get("ticker", "?"),
                "strategy": pos.get("strategy", ""),
    ...
    starting_equity = primary.get("equity", total_equity) - primary.get("total_pnl", 0)
    total_pnl_all = sum(s.get("total_pnl", 0) for s in summaries.values())
    total_pnl_pct = sum(s.get("total_pnl_pct", 0) for s in summaries.values())
    ...
    return {
        ...
        "portfolio": {
            ...
            "total_pnl": total_pnl_all,
            "total_pnl_pct": total_pnl_pct,
            "today_pnl_aud": total_pnl,  # Field name is legacy from ASX days
            "num_open": total_positions,
            "open_positions": all_positions,
            "realized_pnl": sum(s.get("realized_today", 0) for s in summaries.values()),
        },
        "closed_trades": all_closed,
        "benchmark_return_pct": 0,  # Not available from EOD summary
        "benchmark_ticker": "SPY",
        "risk": {
            "exposure_pct": round(100 * (total_equity - total_cash) / total_equity, 1) if total_equity > 0 else 0,
            "max_positions": primary.get("positions", 0),
        },
    }
